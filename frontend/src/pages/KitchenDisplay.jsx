...
const STATUS_CONFIG = {
  ...
  Ready:     { color: '#15803d', bg: '#f0fdf4', border: '#86efac', next: 'Served',    nextLabel: '🍽️ Mark Served',   icon: '✅' },
...
function OrderCard({ order, onStatusChange, updating, isNew }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: urgency ? `0 4px 20px ${urgency}30` : '0 2px 10px rgba(21,128,61,0.07)',
      transition: 'all 0.2s',
      ...
      <div style={{ background: cfg.bg, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${cfg.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 18, color: '#14532d' }}>Table {order.table_no}</div>
            <div style={{ fontSize: 12, color: '#94a3b8' }}>Order #{order.id}</div>
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ background: cfg.bg, color: cfg.color, border: `1.5px solid ${cfg.border}`, borderRadius: 20, padding: '3px 12px', fontSize: 12, fontWeight: 600 }}>
            {order.status}
          </div>
        ...
        {(order.items || []).map((item, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < order.items.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ background: '#15803d', color: 'white', fontWeight: 700, fontSize: 13, width: 26, height: 26, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{item.qty}</span>
              <span style={{ fontWeight: 600, fontSize: 15, color: '#14532d' }}>{item.item_name}</span>
            </div>
      ...
      <div style={{ padding: '8px 16px', background: '#f0fdf4', borderTop: '1px solid #f1f5f9', fontSize: 12, color: '#86a98f' }}>
        ...
        <div style={{ padding: '12px 16px' }}>
          <button
            ...
            style={{
              ...
              fontSize: 14, fontWeight: 600, opacity: updating === order.id ? 0.6 : 1, transition: 'all 0.15s',
              fontFamily: "'Poppins', sans-serif",
            }}>
    ...
    <div className="page-container" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <style>{`
        ...
        @keyframes newOrderPulse {
          0%, 100% { transform: scale(1); box-shadow: 0 2px 10px rgba(21,128,61,0.07); }
      ...
      <div className="page-content">
        ...
        {notification && (
          <div style={{
            ...
            fontWeight: 700, fontSize: 18, zIndex: 9999,
        ...
        {!notification && unacked > 0 && (
          <div style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', borderRadius: 12, padding: '10px 16px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ color: '#ef4444', fontWeight: 600, fontSize: 14 }}>🔔 {unacked} new order{unacked > 1 ? 's' : ''} since last check</span>
            <button onClick={() => setUnacked(0)} style={{ background: '#ef4444', color: 'white', border: 'none', borderRadius: 8, padding: '5px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: "'Poppins', sans-serif" }}>✓ Got it</button>
          </div>
        )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            <h1 style={{ margin: 0, color: '#14532d', fontWeight: 700 }}>🍳 Kitchen Display</h1>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {pendingCount > 0 && <span style={{ background: '#fef2f2', color: '#ef4444', border: '1.5px solid #fca5a5', borderRadius: 20, padding: '4px 12px', fontSize: 13, fontWeight: 600 }}>🆕 {pendingCount} Pending</span>}
              {preparingCount > 0 && <span style={{ background: '#fffbeb', color: '#d97706', border: '1.5px solid #fde68a', borderRadius: 20, padding: '4px 12px', fontSize: 13, fontWeight: 600 }}>🍳 {preparingCount} Cooking</span>}
              {readyCount > 0 && <span style={{ background: '#f0fdf4', color: '#15803d', border: '1.5px solid #86efac', borderRadius: 20, padding: '4px 12px', fontSize: 13, fontWeight: 600 }}>✅ {readyCount} Ready</span>}
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 12, color: '#86a98f' }}>🟢 Live · {lastRefresh.toLocaleTimeString()}</span>
            {/* Sound Toggle */}
            <button
              onClick={() => setSoundOn(prev => !prev)}
              title={soundOn ? 'Mute sound' : 'Unmute sound'}
              style={{ padding: '10px 14px', background: soundOn ? '#f0fdf4' : '#fef2f2', color: soundOn ? '#15803d' : '#ef4444', border: `1.5px solid ${soundOn ? '#86efac' : '#fca5a5'}`, borderRadius: 10, cursor: 'pointer', fontSize: 16, fontWeight: 600 }}>
              {soundOn ? '🔔' : '🔕'}
            </button>
            <button onClick={() => loadOrders()} style={{ padding: '10px 18px', background: '#f0fdf4', color: '#15803d', border: '1.5px solid #bbf7d0', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: "'Poppins', sans-serif" }}>
              🔄 Refresh
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          {filters.map(f => (
            <button key={f} onClick={() => setActiveFilter(f)} style={{ padding: '8px 20px', borderRadius: 20, fontSize: 14, fontWeight: 600, border: `1.5px solid ${activeFilter === f ? '#15803d' : '#e2e8f0'}`, background: activeFilter === f ? '#15803d' : 'white', color: activeFilter === f ? 'white' : '#64748b', cursor: 'pointer', transition: 'all 0.15s', fontFamily: "'Poppins', sans-serif" }}>
              {f}
            </button>
          ))}
        ...
        {loading ? (
          <SkeletonCardGrid count={6} minWidth={300} cardHeight={220} />
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 80, color: '#86a98f' }}>
            <div style={{ fontSize: 64, marginBottom: 16 }}>🍽️</div>
            <div style={{ fontWeight: 700, fontSize: 18, color: '#14532d' }}>All Clear!</div>
