...
<body>
  ...
  const getStockBadge = (item) => {
    ...
    if (s <= 10) return { label: ` ${s} left`, color: #d97706, bg: #fffbeb }
      ...
      <div className="page-content">
        <div style={{ display: flex, justifyContent: space-between, alignItems: center, marginBottom: 16, flexWrap: wrap, gap: 10 }}>
          ...
          <div style={{ display: flex, gap: 6, flexWrap: wrap }}>
            {totalAlerts > 0 && (
              <button onClick={() => setShowAlerts(true)} style={{ padding: isMobile ? 7px 10px : 10px 16px, background: #fffbeb, color: #d97706, border: 1.5px solid #fde68a, borderRadius: 10, cursor: pointer, fontSize: isMobile ? 11 : 13, fontWeight: 600 }}> {totalAlerts}</button>
            )}
        ...
        {showVoidModal && (
          <div className="modal-overlay">
            <div className="modal">
              ...
              <p>Void <strong>#{lastTxId}</strong> and free <strong>Table {lastTableNo}</strong>?</p>
          ...
          <div className="modal-overlay">
            <div className="modal" style={{ maxWidth: 480 }}>
              ...
              {lowStockAlerts.lowStock.length > 0 && (
                <div>
                  <div style={{ fontWeight: 700, color: #d97706, marginBottom: 8 }}> Low Stock ({lowStockAlerts.lowStock.length})</div>
