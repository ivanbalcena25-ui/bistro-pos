...
// Shared helper for comma formatting
...
function Transaction() {
  ...
  const qrRefGCash = useRef()
  const qrRefMaya = useRef()
  const qrRefShopeePay = useRef()
  const qrRefGrabPay = useRef()
  ...
  const handleTableChange = (e) => {
    ...
    if (val) {
      ...
      if (getTableStatus(num) === 'Occupied') setTableError(`Table ${num} is occupied! Choose another.`)
    }
  }
  ...
  const getItemDiscountAmt = (item) => {
    const map = getDiscountMap()
    return getItemSubtotal(item) * (map[item.discount || 'None'] || 0)
  }
  ...
  const handleAmountPaidChange = (e) => {
    ...
    setAmountPaid(raw)
    setAmountPaidDisplay(formatCashInput(raw))
  }
  ...
  const handleOpeningCashChange = (e) => {
    ...
    if (!/^\d*\.?\d*$/.test(raw)) return
    setOpeningCash(raw)
    setOpeningCashDisplay(formatCashInput(raw))
  }
  ...
  const handleClosingCashChange = (e) => {
    ...
    if (!/^\d*\.?\d*$/.test(raw)) return
    setClosingCash(raw)
    setClosingCashDisplay(formatCashInput(raw))
  }
  ...
  const handleCheckout = async () => {
    ...
    try {
      const result = await addTransaction({
        customer_name: `Table ${num}`, table_no: num,
        total, amount_paid: paid, change_amount: change,
  ...
  const handleVoid = async () => {
    ...
    try {
      ...
      setReceipt(null); setLastTxId(null); setLastTableNo(null)
  ...
  const handleOpenShift = async () => {
    try {
      ...
      await loadActiveShift(); setShowShiftModal(false)
      setOpeningCash(''); setOpeningCashDisplay('')
  ...
  const handleReprintById = () => {
    ...
    const r = {
      ...
      cashier_name: tx.cashier_name, date: new Date(tx.created_at).toLocaleString(), voided: false
    }
    printReceipt(r); setShowReprintModal(false); setReprintTxId('')
  }

  const openSettings = () => {
    setTmpSettings(JSON.parse(JSON.stringify(settings)))
  ...
  const handleQrUpload = (wallet, file) => {
    ...
    reader.onload = (e) => {
      setTmpSettings(prev => ({ ...prev, qrImages: { ...prev.qrImages, [wallet]: e.target.result } }))
    }
    reader.readAsDataURL(file)
  }

  const addDiscount = () => {
    setTmpSettings(prev => ({ ...prev, discounts: [...prev.discounts, { label: 'New Discount', rate: 10 }] }))
  }

  const updateDiscount = (idx, field, val) => {
    setTmpSettings(prev => {
  ...
  const removeDiscount = (idx) => {
    setTmpSettings(prev => ({ ...prev, discounts: prev.discounts.filter((_, i) => i !== idx) }))
  }
  ...
  const downloadZReadingExcel = () => {
    ...
    const todayTx = allTransactions.filter(t => new Date(t.created_at).toDateString() === today)
    const validTx = todayTx.filter(t => !t.voided)
    const voidedTx = todayTx.filter(t => t.voided)
    const dateStr = new Date().toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' })
    const wb = XLSX.utils.book_new()
    const summaryData = [
      ['VS HOTEL BISTRO'], ['Z-READING — DAILY SALES REPORT'],
      [`Date: ${dateStr}`], [`Generated: ${new Date().toLocaleString()}`], [`Generated by: ${user.username}`], [],
      ...
      ...['Cash', 'GCash', 'Maya', 'ShopeePay', 'GrabPay', 'Visa', 'Mastercard', 'Credit Card', 'Debit Card'].map(pm => {
    ...
    const itemMap = {}
    validTx.forEach(t => { (t.items || []).forEach(i => { if (!itemMap[i.item_name]) itemMap[i.item_name] = { qty: 0, total: 0 }; itemMap[i.item_name].qty += i.qty; itemMap[i.item_name].total += i.subtotal }) })
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ['Item Name', 'Qty Sold', 'Total Sales'],
      ...Object.entries(itemMap).sort((a, b) => b[1].total - a[1].total).map(([name, d]) => [name, d.qty, d.total])
    ]), 'Item Breakdown')
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
  ...
  const downloadXReadingExcel = () => {
    ...
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ['VS HOTEL BISTRO'], ['X-READING — MID-DAY SNAPSHOT'],
      [`As of: ${now.toLocaleString()}`], [`Generated by: ${user.username}`], [],
  ...
  const subtotal = getSubtotalBeforeVat()
  const vatAmt = getVatAmount()
  const total = getTotal()
      ...
      <div className="page-content">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            {activeShift ? (
              <span style={{ background: '#f0fdf4', border: '1.5px solid #bbf7d0', color: '#16a34a', fontSize: 12, fontWeight: 700, padding: '4px 12px', borderRadius: 20 }}>🟢 Shift #{activeShift.id} Open</span>
            ) : (
              <span style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', color: '#ef4444', fontSize: 12, fontWeight: 700, padding: '4px 12px', borderRadius: 20, cursor: 'pointer' }} onClick={() => setShowShiftModal(true)}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {totalAlerts > 0 && (
              <button onClick={() => setShowAlerts(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 16px', background: '#fffbeb', color: '#d97706', border: '1.5px solid #fde68a', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                ⚠️ {totalAlerts} Stock Alert{totalAlerts > 1 ? 's' : ''}
              </button>
            )}
            <button onClick={openSettings} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 16px', background: '#f8fafc', color: '#475569', border: '1.5px solid #e2e8f0', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              <FaCog size={13} /> Settings
            </button>
            <button onClick={() => setShowReprintModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 16px', background: '#f8fafc', color: '#475569', border: '1.5px solid #e2e8f0', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              <FaPrint size={12} /> Reprint
            </button>
            <button onClick={() => setShowXReading(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 16px', background: '#7c3aed', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              <FaFileAlt size={12} /> X-Reading
            </button>
            <button onClick={() => setShowZReading(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', background: '#1e40af', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
              <FaFileAlt size={12} /> Z-Reading
            </button>
            {activeShift && (
              <button onClick={() => setShowShiftModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 16px', background: '#ef4444', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                Close Shift
              </button>
            )}
          </div>
        ...
        {!activeShift && (
          <div onClick={() => setShowShiftModal(true)} style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', color: '#ef4444', padding: '12px 18px', borderRadius: 12, marginBottom: 16, fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
            <span>🔴 No active shift. Transactions are disabled. Click here to open a shift.</span>
            <span style={{ fontSize: 12, background: '#ef4444', color: 'white', padding: '4px 12px', borderRadius: 20 }}>Open Shift →</span>
          </div>
        )}

        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', background: 'white', padding: '14px 18px', borderRadius: 14, marginBottom: 22, boxShadow: '0 1px 4px rgba(22,163,74,0.08)', border: '1px solid #d1fae5' }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: '#16a34a', alignSelf: 'center', marginRight: 4 }}>Tables:</span>
          {Array.from({ length: TOTAL_TABLES }, (_, i) => i + 1).map(num => {
            const occupied = getTableStatus(num) === 'Occupied'
            return (
              <div key={num} style={{ padding: '5px 13px', borderRadius: 20, fontSize: 13, fontWeight: 700, background: occupied ? '#fef2f2' : '#f0fdf4', color: occupied ? '#ef4444' : '#16a34a', border: `1.5px solid ${occupied ? '#fca5a5' : '#bbf7d0'}` }}>
                {occupied ? '🔴' : '🟢'} T{num}
              </div>
            )
          })}
          ...
          <div className="cart-panel">
            ...
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '10px 14px', marginTop: 4 }}>
              {settings.vatEnabled && (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#64748b', marginBottom: 4 }}>
                    <span>VAT ({settings.vatRate}%)</span><span>₱{vatAmt.toFixed(2)}</span>
                  </div>
                </>
              )}
            ...
            {!activeShift && (
              <div style={{ background: '#fef2f2', border: '1.5px solid #fca5a5', color: '#ef4444', padding: '10px 14px', borderRadius: 10, fontSize: 13, fontWeight: 600, textAlign: 'center' }}>
                🔴 Open a shift first to accept transactions.
              </div>
            )}
        ...
        {receipt && !receipt.voided && (
          <div className="receipt" style={{ marginTop: 26 }}>
            ...
            {receipt.vat_enabled && (
              <>
                <div className="receipt-item"><span>Subtotal</span><span>₱{Number(receipt.subtotal).toLocaleString()}</span></div>
                <div className="receipt-item" style={{ color: '#64748b' }}><span>VAT ({receipt.vat_rate}%)</span><span>₱{Number(receipt.vat_amount).toFixed(2)}</span></div>
              </>
            )}
            ...
            <div className="modal" style={{ maxWidth: 540, maxHeight: '85vh', overflowY: 'auto' }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
                    <input type="checkbox" checked={tmpSettings.vatEnabled}
                      onChange={e => setTmpSettings(prev => ({ ...prev, vatEnabled: e.target.checked }))}
                ...
                {tmpSettings.vatEnabled && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <input type="number" min="0" max="100" value={tmpSettings.vatRate}
                      onChange={e => setTmpSettings(prev => ({ ...prev, vatRate: parseFloat(e.target.value) || 0 }))}
                      style={{ width: 80, padding: '6px 10px', border: '1.5px solid #d1fae5', borderRadius: 8, fontSize: 14, outline: 'none' }} />
              ...
              <div style={{ marginBottom: 24 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', marginBottom: 12, paddingBottom: 6, borderBottom: '1.5px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={addDiscount} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', background: '#16a34a', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
                    <FaPlus size={10} /> Add
                  </button>
                </div>
                {tmpSettings.discounts.map((d, idx) => (
                  <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                    <input value={d.label} onChange={e => updateDiscount(idx, 'label', e.target.value)}
                      placeholder="Label (e.g. Senior)" style={{ flex: 1, padding: '7px 10px', border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 13, outline: 'none' }} />
                    <input type="number" min="0" max="100" value={d.rate} onChange={e => updateDiscount(idx, 'rate', parseFloat(e.target.value) || 0)}
                      style={{ width: 64, padding: '7px 8px', border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 13, outline: 'none', textAlign: 'center' }} />
                    <span style={{ fontSize: 12, color: '#64748b', minWidth: 14 }}>%</span>
                    <button onClick={() => removeDiscount(idx)} style={{ width: 28, height: 28, background: '#fef2f2', color: '#ef4444', border: 'none', borderRadius: 7, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <FaTrash size={11} />
                    </button>
                  </div>
                ))}
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                {E_WALLETS.map(wallet => (
                  <div key={wallet} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, padding: '10px 12px', background: '#f8fafc', borderRadius: 10, border: '1px solid #e2e8f0' }}>
                    <img src={tmpSettings.qrImages[wallet]} alt={wallet}
                      style={{ width: 52, height: 52, objectFit: 'cover', borderRadius: 8, border: '1.5px solid #e2e8f0', background: 'white' }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{wallet}</div>
                      <button onClick={() => qrInputRefs[wallet].current.click()}
                        style={{ padding: '5px 14px', background: '#eff6ff', color: '#1d4ed8', border: '1.5px solid #bfdbfe', borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                        📂 Upload QR Image
                      </button>
                      <input ref={qrInputRefs[wallet]} type="file" accept="image/*" style={{ display: 'none' }}
                        onChange={e => handleQrUpload(wallet, e.target.files[0])} />
                    </div>
                  </div>
                ))}
              </div>
              <div className="form-actions" style={{ marginTop: 20 }}>
                <button onClick={saveSettingsAndClose} style={{ padding: '10px 24px', background: '#16a34a', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>
                  💾 Save Settings
                </button>
        ...
        {showQRModal && (
          <div className="modal-overlay">
            <div className="modal" style={{ textAlign: 'center', maxWidth: 360 }}>
              ...
              <p style={{ color: '#64748b', fontSize: 14, marginBottom: 16 }}>Ask the customer to scan this QR code to pay <strong>₱{receipt?.total?.toLocaleString()}</strong></p>
              ...
              <div className="form-actions" style={{ justifyContent: 'center' }}>
                <button onClick={() => { setShowQRModal(false); printReceipt(receipt) }} style={{ padding: '11px 24px', background: '#16a34a', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>✅ Paid — Print Receipt</button>
        ...
        {showVoidModal && (
          <div className="modal-overlay">
            <div className="modal">
              ...
              <p>This will void transaction <strong>#{lastTxId}</strong> (₱{receipt?.total?.toLocaleString()}) and free <strong>Table {lastTableNo}</strong>.</p>
              <div style={{ marginTop: 14 }}>
                <label style={lbl}>Reason for Void *</label>
        ...
        {showShiftModal && (
          <div className="modal-overlay">
            <div className="modal" style={{ maxWidth: 400 }}>
              ...
              {activeShift ? (
                <>
                  <p>Shift <strong>#{activeShift.id}</strong> opened at <strong>{new Date(activeShift.opened_at).toLocaleString()}</strong></p>
                  <p style={{ marginTop: 8, color: '#64748b', fontSize: 14 }}>Opening Cash: <strong>₱{Number(activeShift.opening_cash).toLocaleString()}</strong></p>
                  <div style={{ marginTop: 14 }}>
        ...
        {showReprintModal && (
          <div className="modal-overlay">
            <div className="modal" style={{ maxWidth: 380 }}>
              ...
              <p style={{ color: '#64748b', fontSize: 14, marginBottom: 14 }}>Enter the transaction/receipt number to reprint.</p>
              <div>
            ...
            <div className="modal-overlay">
              <div className="modal" style={{ width: 480 }}>
                ...
                <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14 }}><span>Cashier(s)</span><span style={{ fontWeight: 700 }}>{z.cashiers.join(', ') || '—'}</span></div>
                ...
                {z.byCashier.length > 1 && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ fontWeight: 700, fontSize: 13, color: '#475569', marginBottom: 6 }}>Per Cashier</div>
                    {z.byCashier.map(c => (
                      <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '6px 0', borderBottom: '1px solid #f1f5f9' }}>
                        <span>🧑‍💼 {c.name} ({c.count} tx)</span><span style={{ fontWeight: 700, color: '#16a34a' }}>₱{c.total.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        {showXReading && (() => {
          ...
          const validTx = currentTx.filter(t => !t.voided)
          const total = validTx.reduce((s, t) => s + Number(t.total), 0)
          return (
            <div className="modal-overlay">
              <div className="modal" style={{ width: 440 }}>
                <h2>📈 X-Reading — Mid-Day Snapshot</h2>
                <p style={{ color: '#64748b', fontSize: 13, marginBottom: 14 }}>Non-resetting snapshot as of {now.toLocaleTimeString()}. This does NOT close the day.</p>
                <div style={{ background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 12, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14 }}><span>Transactions so far</span><span style={{ fontWeight: 700 }}>{validTx.length}</span></div>
                  ...
                  <div style={{ borderTop: '1px dashed #ddd6fe', paddingTop: 10, display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontWeight: 800, color: '#7c3aed', fontSize: 18 }}>₱{total.toLocaleString()}</span>
                  </div>
                </div>
                <div className="form-actions" style={{ marginTop: 16 }}>
